...
function renderFieldsList(fields) {
  ...
  // Render fields
  fieldsListEl.innerHTML = fields.map(field => {
      ...
      <div class="p-4 border border-[var(--cane-200)] rounded-lg flex items-center justify-between hover:bg-[var(--cane-50)] transition-colors">
        <div class="flex-1">
          <div class="flex items-center gap-2 mb-1">
            ...
            <p class="font-semibold text-[var(--cane-900)] text-lg">${escapeHtml(fieldName)}</p>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-2 mt-2 text-sm text-[var(--cane-600)]">
            <p><i class="fas fa-ruler-combined text-[var(--cane-400)] mr-2"></i>Area: ${escapeHtml(area)} hectares</p>
            <p><i class="fas fa-seedling text-[var(--cane-400)] mr-2"></i>Variety: ${escapeHtml(variety)}</p>
            ${location ? `<p class="md:col-span-2"><i class="fas fa-location-dot text-[var(--cane-400)] mr-2"></i>${escapeHtml(location)}</p>` : ''}
          </div>
        </div>
        <button onclick="viewFieldDetails('${field.id}')" class="ml-4 px-4 py-2 bg-[var(--cane-600)] hover:bg-[var(--cane-700)] text-white rounded-lg transition-colors font-medium">
...
function renderTasksList(tasks) {
  ...
  if (filteredTasks.length === 0) {
    tasksList.innerHTML = `
      <div class="text-center py-12 text-gray-500">
        ...
        <p class="text-lg font-medium">No ${filter === 'all' ? '' : filter} tasks</p>
      </div>
    `;
    return;
  }
  ...
  tasksList.innerHTML = filteredTasks.map(task => {
    ...
    const statusIcon = task.status === 'done' ? 'fa-check-circle' : 'fa-clock';
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex-1">
            <div class="flex items-center gap-2 mb-2">
              <h3 class="font-semibold text-[var(--cane-900)]">${task.title}</h3>
    ...
    collapseBtn.addEventListener('click', () => {
      ...
      if (sidebar && mainContent) {
        ...
        if (isCollapsed) {
          ...
          const userInfo = sidebar.querySelector('.flex.items-center.space-x-3.mb-5 > div:last-child');
...
async function loadSectionData(sectionId) {
  switch (sectionId) {
    ...
    case 'my-fields':
      ...
      console.log('Rendering fields section with', currentFields.length, 'fields');
...
async function loadDashboardData() {
  try {
    ...
    if (activeFieldsCount) activeFieldsCount.textContent = stats.totalFields || 0;
    if (totalTasksCount) totalTasksCount.textContent = stats.totalTasks || 0;
    if (pendingTasksCount) pendingTasksCount.textContent = stats.pendingTasks || 0;
    if (rentalRequestsCount) rentalRequestsCount.textContent = stats.pendingRentalRequests || 0;
...
async function loadRecentActivity() {
  ...
  try {
    ...
    activityList.innerHTML = recentTasks.map(task => {
      ...
      const icon = task.status === 'done' ? 'fa-check-circle text-green-600' : 'fa-clock text-yellow-600';

      return `
        <div class="py-3 flex items-start justify-between">
          <div class="flex items-start gap-2">
            <i class="fas ${icon} mt-1"></i>
            <span class="text-[var(--cane-800)]">${action}: ${task.title || task.taskType || 'Task'}</span>
          </div>
          ...
          <div class="flex flex-col gap-3">
            <div class="flex items-start justify-between">
              <div class="flex-1">
                ...
                ${rental.remarks ? `<p class="text-sm text-gray-500 mt-2 italic">"${escapeHtml(rental.remarks)}"</p>` : ''}
              </div>
            </div>
            ${isPending ? `
...
function setupProfileDropdown() {
  ...
  newProfileBtn.addEventListener('click', (e) => {
    ...
    if (isHidden) {
      ...
      newProfileBtn.querySelector('.fa-chevron-down')?.classList.add('rotate-180');
    } else {
      // Hide dropdown
      profileDropdown.classList.add('opacity-0', 'invisible', 'scale-95');
      newProfileBtn.querySelector('.fa-chevron-down')?.classList.remove('rotate-180');
    }
  });
...
window.viewFieldDetails = async function(fieldId) {
  ...
  try {
    ...
    const { doc, getDoc, collection, query, where, getDocs } = await import('https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js');
        ...
        <div class="p-6 overflow-y-auto max-h-[calc(90vh-5rem)]">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
              <div class="flex items-start gap-2">
                ...
                <div>
                  ...
                  <span class="ml-1 font-medium text-gray-900">${escapeHtml(area)} hectares</span>
                </div>
              </div>
              <div class="flex items-start gap-2">
                ...
                <div>
                  ...
                  <span class="ml-1 font-medium text-gray-900">${escapeHtml(variety)}</span>
                </div>
              </div>
              ${barangay ? `
              <div class="flex items-start gap-2">
                ...
                <div>
                  ...
                  <span class="ml-1 font-medium text-gray-900">${escapeHtml(barangay)}</span>
                </div>
              </div>
              ` : ''}
              ${location ? `
              <div class="flex items-start gap-2 md:col-span-2">
                ...
                <div>
                  ...
                  <span class="ml-1 font-medium text-gray-900">${escapeHtml(location)}</span>
                </div>
              </div>
              ` : ''}
            </div>
          ...
          <div>
            ...
            ` : `
              <div class="space-y-2">
                ${myTasks.map(task => {
                  const statusColor = task.status === 'done' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800';
                  const timeField = task.deadline || task.createdAt;
                  const dateStr = timeField ? (timeField.toDate ? timeField.toDate() : new Date(timeField)).toLocaleDateString() : '—';
                  ...
                  return `
                    <div class="border border-gray-200 rounded-lg p-3 hover:shadow-md transition">
                      <div class="flex items-start justify-between">
                        <div class="flex-1">
                          <div class="flex items-center gap-2 mb-1">
                            <span class="font-semibold text-gray-900">${escapeHtml(task.title || 'Task')}</span>
                            <span class="px-2 py-1 rounded-full text-xs font-medium ${statusColor}">
                              ${task.status || 'pending'}
                            </span>
                          </div>
    ...
    modal.querySelector('#closeFieldModal').addEventListener('click', closeModal);
    modal.querySelector('#closeFieldModalBtn').addEventListener('click', closeModal);
...
window.markDriverTaskAsDone = async function(taskId) {
  try {
    ...
    if (fieldId && handlerId) {
      ...
      console.log(`📋 Driver marked task as done - Title: "${task.title}", Field: ${fieldId}`);
...
window.openDriverLogWorkModal = async function() {
  try {
    ...
    const fieldsOptions = fields.map(f =>
      `<option value="${f.id}">${escapeHtml(f.fieldName || f.name || 'Unknown Field')}</option>`
    ).join('');
    ...
    const { value: formValues } = await Swal.fire({
      ...
      preConfirm: () => {
        ...
        return { fieldId, taskType, completionDate, driverName, notes, photoFile, verification };
      }
    });
...
async function createDriverLog(logData) {
  ...
  try {
    ...
    const { db } = await import('../Common/firebase-config.js');
    const { collection, addDoc, serverTimestamp, Timestamp, doc, getDoc } = await import('https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js');
    ...
    console.log(`📋 Driver log created - Task: "${logData.taskType}", Field: ${logData.fieldId}, Variety: ${fieldVariety}`);
    ...
    // Check if this is basal fertilization (handles: "basal_fertilizer", "basal fertilizer", "Basal Fertilizer (0–30 DAP)")
      ...
      if (handlerId && logData.fieldId) {
        try {
          console.log(`🌿 Triggering basal fertilization completion for field ${logData.fieldId}`);
          ...
          console.error('❌ Error triggering basal fertilization tracking:', error);
          console.error('Error details:', error.message, error.stack);
        }
      }
    }
    // Check if this is main fertilization (handles: "main_fertilization", "main fertilization", "Main Fertilization (45–60 DAP)")
      ...
      if (handlerId && logData.fieldId) {
        try {
          console.log(`🌾 Triggering main fertilization completion for field ${logData.fieldId}`);
          ...
          console.error('❌ Error triggering main fertilization tracking:', error);
          console.error('Error details:', error.message, error.stack);
        }
      }
    }
